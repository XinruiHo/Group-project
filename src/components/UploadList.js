...
import {uploadToIPFS,uploadJSON} from '../utils/NFTStorageUpload.js'
import JJAX from '../JJAX.json';
import { Input, InputNumber, Upload, Form, Button, Typography,Layout} from 'antd';
...
export default function UploadNFT () {
    ...
    async function uploadMetadataToIPFS() {
        ...
        try {
            ...
            const response = await uploadJSON(nftJSON);
            if(response.success === true){
    ...
    async function listNFT(e) {
        ...
        try {
            ...
            const decimals = 18;
            const bigprice = ethers.utils.parseUnits(formParams.price.toString(), decimals);
            console.log('bignumber created')

            const tokenId = events[0].args.tokenId.toNumber();
                

            const createlist = await contract.createFixedPriceListing(tokenId,bigprice);
